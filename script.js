...
// Full rewrite: emojis, reactions, reply, edit, fixed delete
// ============================================================
...
// Reply & edit state
let dmReplyTo = null;       // { id, name, text }
let groupReplyTo = null;
let dmEditingId = null;     // message id being edited
let groupEditingId = null;
let groupEditingOriginalText = '';
...
// Reactions cache: { messageId: { emoji: { uid: true } } }
let dmReactions = {};
let groupReactions = {};
...
// FIXED: bulletproof against undefined / NaN / null
...
// Preview text for a message (used in rail + reply + edit)
...
signupBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        await usersRef.child(user.uid).set({
            ...
            avatar: '', banner: '', bio: '', status: 'online'
        });
    } catch (err) {
        ...
        else if (err.code === 'PERMISSION_DENIED' || (err.message || '').includes('PERMISSION_DENIED')) msg = 'Database permission denied.';
...
async function removeFriend(uid) {
    ...
    const dname = u?.displayName || u?.username || 'this friend';
...
contextMenu.querySelectorAll('.context-item').forEach(item => {
    item.addEventListener('click', async e => {
        ...
        if (action === 'profile') {
            ...
            if (u) removeFriend(uid);
        } else if (action === 'delete') {
            // FIXED DELETE CONVERSATION
            const ok = await showModal({
            ...
            if (!ok) return;
            try {
                // Remove DM messages between the two users
                await dmsRef.child(dmKey(userId, uid)).remove();

                // Clear unread for both sides
                await unreadRef.child(userId).child(key).remove();
                await unreadRef.child(uid).child('dm_' + userId).remove();

                // Clear cached last message
                delete lastMessages['dm_' + dmKey(userId, uid)];
                _lastRailHash = '';

                // Clear reactions on that conversation
                try { await reactionsRef.child(dmKey(userId, uid)).remove(); } catch (e) {}

                // If we're currently viewing this conversation, close it
...
function updateMuteButtons() {
    if (currentDmUser) {
        ...
        dmMuteBtn.innerHTML = m ? '<i class="fas fa-bell-slash"></i>' : '<i class="fas fa-bell"></i>';
    }
    if (currentGroup) {
        const m = isChatMuted('group_' + currentGroup.id);
        groupMuteBtn.classList.toggle('active-mute', m);
        groupMuteBtn.innerHTML = m ? '<i class="fas fa-bell-slash"></i>' : '<i class="fas fa-bell"></i>';
    }
}
...
async function openDM(uid, dname, userData) {
    ...
    // Listen to reactions for this conversation
    ...
    const cb = snap => {
        ...
        if (!msg) return;
        msg._id = snap.key;
        // Avoid duplicate render if already present (e.g. after edit)
        ...
        if (existing) {
            ...
            // Add edited marker if needed
            const header = existing.querySelector('.msg-header');
            let editedMark = header.querySelector('.msg-edited');
            if (msg.edited && !editedMark) {
...
// Build a quote block for replied messages
...
// ============================================================
// DELEGATED CLICKS — copy / delete / reply / edit / react
// ============================================================
document.addEventListener('click', async e => {
    ...
    if (action === 'copy') {
        ...
        if (copied) showToast('Copied', 'Message copied to clipboard.');
        else showToast('Could not copy', 'Your browser blocked clipboard access.');
        ...
        if (source === 'dm') {
            ...
            groupEditingOriginalText = msgText;
...
async function sendDm() {
    ...
    // If editing → save edit instead of sending
    if (dmEditingId) {
        const newText = text;
        if (!newText) return;
        const key = dmKey(userId, currentDmUser.uid);
...
// FIXED DELETE CONVERSATION button in header
...
$('#addMembersConfirmBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        console.error(e);
        showModal({ title: 'Failed', text: 'Could not add members.', icon: 'fas fa-exclamation-triangle', danger: true, confirmText: 'OK', hideCancel: true });
    } finally {
        btn.disabled = false;
        btn.textContent = 'Add';
    }
...
function renderEmojiCategory(cat) {
    ...
    emojiGrid.querySelectorAll('.emoji-item').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Insert at cursor position in whichever input is active
...
// PROFILE VIEW
// ============================================================
async function viewProfile(usernameToView) {
    try {
        const snap = await usersRef.child('usernames').child(usernameToView).once('value');
        if (!snap.exists()) { showToast('Not found', 'That user no longer exists.'); return; }
        const uid = snap.val();
        ...
        const labels = { online: 'Active now', idle: 'Idle', dnd: 'Do not disturb', offline: 'Offline' };
...
function openEditProfile() {
    ...
    if (editState.avatar) {
        ...
        editAvatar.innerHTML = getInitial(displayName) + '<div class="avatar-edit-overlay"><i class="fas fa-camera"></i></div>';
    }
...
// Settings buttons (existing + new export/clear)
$$('.settings-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        if (action === 'edit-displayname' || action === 'edit-username' || action === 'edit-bio') {
            ...
            // NEW: Export profile data as JSON
            try {
                const data = {
